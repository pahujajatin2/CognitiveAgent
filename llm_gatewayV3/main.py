...
print(f"[debug] Looking for .env at: {ROOT.parent / '.env'}")
...
async def lifespan(app: FastAPI):
    ...
    app.state.router_providers = P.build_router_providers(app.state.cache)
    app.state.router_pool = RouterPool(app.state.router_providers, ROUTER_ORDER)
    print(f"[debug] Providers: {list(app.state.providers.keys())}")
    print(f"[debug] Routers: {list(app.state.router_providers.keys())}")
    yield
...
async def chat(req: ChatRequest):
    ...
    print(f"[debug] 503 error: {last_err}, attempts: {all_attempts}")
